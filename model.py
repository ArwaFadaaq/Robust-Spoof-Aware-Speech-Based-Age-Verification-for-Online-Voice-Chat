...
This module implements the training and evaluation pipeline for a
two-head speech model designed for age classification and spoof detection.
...
- An encoder (WavLM + LoRA)
- Two classification heads:
  • Age head   → minor vs adult
  • Spoof head → clean vs spoof

Rather than modifying the architecture across experiments, different
training behaviors are controlled using loss weights.

Workflow
--------
1. Load audio segments from CSV manifests
2. Apply optional preprocessing (normalization / transforms)
...
----------------
- Clean baseline:
  Train only age classification (spoof loss disabled)

- Noise augmentation:
  Inject noise dynamically during training

- Multi-task training:
  Train both age and spoof detection jointly

- Robustness testing:
  Evaluate performance under noisy or spoofed conditions

Notes
-----
- Audio is assumed to be preprocessed (mono, 16 kHz, fixed length)
- The same model structure is reused across all experiments
- Flexibility is achieved through configuration and data input
"""
...
class SpeechManifestDataset(Dataset):
        ...
        for d in datasets:
            ...
            for _, row in df.iterrows():
                ...
                self.data.append({
                    ...
                    "path": row[audio_col], # audio file path
                    "age": row[age_col], # age label
                    "spoof": spoof
                })
    ...
    def __getitem__(self, idx):
        ...
        age_label = self._encode_age(item["age"])
        spoof_label = item["spoof"]
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            for i in range(len(batch["segment_id"])):
                epoch_samples.append({
                    ...
                    "age_label": int(batch["age_label"][i].item()),
                    "spoof_label": int(batch["spoof_label"][i].item()),
                    "noise_type": batch["noise_type"][i],
                    "SNR_db": batch["SNR_db"][i]
                })
...
def evaluate_model(config, test_datasets, checkpoint_path, test_transform=None,
    ...
    """
    Evaluate the trained model on age and spoof predictions.

    If spoof_col is None, spoof metrics are not meaningful because all samples
    are assumed to be clean. Age metrics are always reported.
    """
    ...
    # Recreate model architecture before loading saved weights.
    model = MultiTaskWavLM(config).to(device)
    ...
    age_preds, age_labels_all = [], []
    spoof_preds, spoof_labels_all = [], []

    # Report spoof metrics only when spoof labels are available.
    has_spoof = any(d.get("spoof_col") is not None for d in test_datasets)

    # Disable gradients during testing.
    with torch.no_grad():
        for batch in test_loader:
            # Move input waveform to device.
            waveforms = batch["input_values"].to(device)
            ...
            if eval_spoof and has_spoof:
                spoof_labels = batch["spoof_label"].to(device)
                spoof_pred = torch.argmax(outputs["spoof_logits"], dim=1)

                spoof_preds.extend(spoof_pred.cpu().numpy())
                spoof_labels_all.extend(spoof_labels.cpu().numpy())

    # Report age classification metrics.
    print("\nAge Classification Results")
    print("-" * 40)
    print("Accuracy:", accuracy_score(age_labels_all, age_preds))
    print("Macro-F1:", f1_score(age_labels_all, age_preds, average="macro"))
    print(classification_report(age_labels_all, age_preds, target_names=["minor", "adult"]))
    print("Confusion Matrix:")
    print(confusion_matrix(age_labels_all, age_preds))

    if eval_spoof and has_spoof:
        print("\nSpoof Detection Results")
        print("-" * 40)
        print("Accuracy:", accuracy_score(spoof_labels_all, spoof_preds))
        print("Macro-F1:", f1_score(spoof_labels_all, spoof_preds, average="macro"))
        print(classification_report(spoof_labels_all, spoof_preds, target_names=["clean", "spoof"]))
        print("Confusion Matrix:")
        print(confusion_matrix(spoof_labels_all, spoof_preds))

    results = {
        "age_accuracy": accuracy_score(age_labels_all, age_preds),
        "age_macro_f1": f1_score(age_labels_all, age_preds, average="macro"),
        "age_preds": age_preds,
        "age_labels": age_labels_all,
    }

    if eval_spoof and has_spoof:
        results.update({
            "spoof_accuracy": accuracy_score(spoof_labels_all, spoof_preds),
            "spoof_macro_f1": f1_score(spoof_labels_all, spoof_preds, average="macro"),
            "spoof_preds": spoof_preds,
            "spoof_labels": spoof_labels_all,
        })
