...
class ProgressTracker:
    ...
    def _load(self):
        try:
            ...
            for _, r in prog.iterrows():
                self._done.add((str(r["segment_id"]), str(r["spoof_engine"])))
        except:
            pass
        ...
        try:
            ...
            for _, r in fail.iterrows():
                self._engine_bad.setdefault(str(r["segment_id"]), set()).add(str(r["spoof_engine"]))
        except:
            pass
    ...
    def mark_done(self, segment_id, engine, final_path):
        ...
        if key in self._done:
            return
        self._done.add(key)
        pd.DataFrame([[str(segment_id), str(engine), final_path]],
    ...
    def mark_engine_failed(self, segment_id, engine, error_msg, spoof_type):
        ...
        pd.DataFrame([[str(segment_id), str(engine), str(error_msg)[:300], spoof_type]],
...
def build_source_pools(source_df, valid_seg_ids, rng):
    shuffled = source_df.sample(frac=1, random_state=int(rng.integers(1e9))).reset_index(drop=True)
        ...
        with ManifestWriter(manifest_path, flush_every=5) as mw:
            ...
            while queue:
                ...
                if kind != "replay":
                    ...
                    if target_row is None:
                        tracker.mark_engine_failed(
                            seg_id, engine, "no target speaker", kind
                        )
                        _reassign_segment(row, engine, type_engines, by_engine, queue)
                        fail_count += 1
                        pbar.update(1)
                        continue
                ...
                if not os.path.exists(src_path):
                    tracker.mark_engine_failed(
                        seg_id, engine, "missing source", kind
                    )
                    fail_count += 1
                    pbar.update(1)
                    continue

                text = transcript_lookup.get(seg_id) if kind == "tts" else None
                ...
                if not success:
                    tracker.mark_engine_failed(
                        seg_id, engine, last_err or "error", kind
                    )
