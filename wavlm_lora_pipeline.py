...
def summarize_metrics_for_filters(
    ...
    # Infer dataset source from segment_id
    def infer_dataset_source(segment_id):

        segment_id = str(segment_id)

        if segment_id.startswith("common_voice_"):
            return "common_voice"

        elif segment_id.startswith("myst_"):
            return "myst"

        elif segment_id.startswith("voxceleb_"):
            return "voxceleb"
...
# =========================================================
# Evaluation Filter Definitions
# =========================================================

"""
Shared evaluation filters used across experiments.

These filters define standardized evaluation subsets used to measure:
- real age robustness under noise
- spoof robustness by attack subtype
- cross-age spoofing robustness
- potential dataset-source shortcut learning
- seen vs held-out spoof engine generalization

The filters are applied to the prediction DataFrame generated by
`evaluate_model()`.
"""

# =========================================================
# Real Age Robustness Filters
# =========================================================

REAL_AGE_FILTERS = {

    "test_real_clean": lambda df: (
        (df["authenticity"] == "real") &
        (df["snr_db"].astype(str) == "clean")
    ),

    "test_real_noisy_20dB": lambda df: (
        (df["authenticity"] == "real") &
        (df["snr_db"].astype(str) == "20")
    ),

    "test_real_noisy_10dB": lambda df: (
        (df["authenticity"] == "real") &
        (df["snr_db"].astype(str) == "10")
    ),

    "test_real_noisy_5dB": lambda df: (
        (df["authenticity"] == "real") &
        (df["snr_db"].astype(str) == "5")
    ),
}


# =========================================================
# Spoof Robustness by Subtype
# =========================================================

SPOOF_SUBTYPE_FILTERS = {

    "test_spoof_vc_clean": lambda df: (
        (df["authenticity"] == "spoof") &
        (df["spoof_type"] == "vc") &
        (df["snr_db"].astype(str) == "clean")
    ),

    "test_spoof_tts_clean": lambda df: (
        (df["authenticity"] == "spoof") &
        (df["spoof_type"] == "tts") &
        (df["snr_db"].astype(str) == "clean")
    ),

    "test_spoof_replay_clean": lambda df: (
        (df["authenticity"] == "spoof") &
        (df["spoof_type"] == "replay") &
        (df["snr_db"].astype(str) == "clean")
    ),

    "test_spoof_vc_noisy_10dB": lambda df: (
        (df["authenticity"] == "spoof") &
        (df["spoof_type"] == "vc") &
        (df["snr_db"].astype(str) == "10")
    ),

    "test_spoof_tts_noisy_10dB": lambda df: (
        (df["authenticity"] == "spoof") &
        (df["spoof_type"] == "tts") &
        (df["snr_db"].astype(str) == "10")
    ),
...
# =========================================================
# Seen vs Held-Out Engine Generalization
# =========================================================

"""
Seen vs Held-Out Engine Generalization

Each spoof-aware experiment was trained using a different spoof engine split.

- Seen engines:
  engines observed during training/validation

- Held-out engines:
  completely unseen engines excluded from training

These filters evaluate the model's ability to generalize to unseen
spoofing methods.
"""

# ---------------------------------------------------------
# B2 → trained on spoof set1
# Seen:
#   koko_tts
#   koko_vc
#   openvoice_vc
...
ENGINE_FILTERS_B2 = {
    ...
    "test_seen_B2": lambda df: (
        df["spoof_engine"].isin([
            "koko_tts",
            "koko_vc",
            "openvoice_vc",
            "replay_c1",
    ...
    "test_heldout_B2_vc": lambda df: (
        df["spoof_engine"].isin([
            "seed_vc",
        ])
    ),

    "test_heldout_B2_tts": lambda df: (
        df["spoof_engine"].isin([
            "chatterbox_tts",
        ])
    ),

    "test_heldout_B2_replay": lambda df: (
        df["spoof_engine"].isin([
            "replay_c3",
        ])
    ),
